...
import gov.me.irs.scheduler.daily.nti.service.SmsKoffSendTaskService;
...
@Slf4j
@Component
public class SmsKoffSendTask {
	
	private final SmsKoffSendTaskService smsKoffSendTaskService;
    ...
    public void runSmsSendTask() {
    	...
    	if(!ObjectUtils.isEmpty(profiles) && Const.PROFILES.PRD.equals(profiles)) {
    		...
    		if(holidayCheck.isHoliday(today) == false) {
    			// 착수지연 알림 대상
    			List<Map<String, Object>> kOffList = smsKoffSendTaskService.selectSmsSendKOffList();
    			if(kOffList.size() > 0 && kOffList != null) {
    				for (Map<String, Object> map : kOffList) {
    					log.debug("[kOffList map][{}]", map);
    					smsKoffSendTaskService.insertSmsTrgt(map);
    				}
    			}
    			
    			List<Map<String, Object>> trgtList = smsKoffSendTaskService.selectTrgt();
    			...
    			for (Map<String, Object> trgtMap : trgtList) {
    				...
    				if(result == true) {
    					// smsKoffSendTaskService.updateTrgt(trgtMap);
    				}
    			}
