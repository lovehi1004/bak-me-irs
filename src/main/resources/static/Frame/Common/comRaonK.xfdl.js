(function()
{
    return function()
    {
        ...
        ***********************************************************************************/
         ...
         * 										"add,remove,remove_all,download,download_all,custom_viewer|문서보기";	// edit 모드일때 버튼리스트
         *										"download,download_all,custom_viewer|문서보기";						// view 모드일때 버튼리스트 , (default)
         *										업로드가 편집모드일 때 버튼바 영역에 노출 할 버튼을 설정합니다. '0'으로 설정시 버튼 영역을 표시하지 않습니다.
         *										add : 파일추가, add_folder : 폴더추가, send : 전송하기, remove : 항목제거, remove_all : 전체 항목제거,
         *										move_first : 맨 앞으로, move_forward : 앞으로, move_back : 뒤로, move_end : 맨 뒤로 ',(콤마)'로 구분하여 설정합니다.
         *										업로드가 보기모드일 때 버튼바 영역에 노출 할 버튼을 설정합니다. '0'으로 설정시 버튼 영역을 표시하지 않습니다.
        ...
        */
        this.fnInitRaonK = function(objForm, objParam)
        {
        	...
        	{
        		this.fvButtonList = objParam.sButtonList;				// "download,download_all,custom_viewer|문서보기" (default)
        	}
        	else
        	{
        		if(this.fvMode == "E")
        		{
        			this.fvButtonList = this.fvEditButtonList;			// "add,remove,remove_all,download,download_all,custom_viewer|문서보기"
        		}
        		else
        		{
        			this.fvButtonList = this.fvViewButtonList;			// "download,download_all,custom_viewer|문서보기"
        		}
        	}
