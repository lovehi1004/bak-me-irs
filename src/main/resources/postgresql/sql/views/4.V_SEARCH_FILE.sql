CREATE OR REPLACE VIEW V_SEARCH_FILE AS
/******************************************************************************
...
SELECT
  ...
  FROM COM_FILE_GROUP_BSC A
 INNER JOIN COM_FILE_DTL B
 	  ...
 	  FROM (
		...
		FROM (
				SELECT VSASB.ORGT_CNCLD_FLMNO, VSASB.KRN_CNCLD_FLMNO
				  FROM V_SEARCH_AGR_STIPL_BSC VSASB
			 ) A
		...
		UNION ALL
	 	...
	 	/* 방법론 뷰 파일정보 조회 */
		SELECT VSHB.EXPLN_FLMNO AS FILE_GROUP_MGNO FROM V_SEARCH_MHDLG_BSC VSHB
		
		UNION ALL
		
		/* 국제감축사업 뷰 파일정보 조회 */
		SELECT VSIB.BIZ_PLND_FLMNO AS FILE_GROUP_MGNO FROM V_SEARCH_IRB_BSC VSIB
 	  ) U
  ) D
    ON D.FILE_GROUP_MGNO = A.FILE_GROUP_MGNO
;

COMMENT ON VIEW V_SEARCH_FILE IS '첨부파일 뷰';
COMMENT ON COLUMN V_SEARCH_FILE.FILE_GROUP_MGNO IS '파일그룹관리번호';
COMMENT ON COLUMN V_SEARCH_FILE.FILE_MGNO IS '파일관리번호';
COMMENT ON COLUMN V_SEARCH_FILE.FILE_FULL_PATH IS '실제파일전체경로';
COMMENT ON COLUMN V_SEARCH_FILE.ORGNL_FILE_NM IS '원본파일명';
